...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (id == BLOCK_WATER_FLOW || id == BLOCK_WATER_STILL) {
    if (id == BLOCK_WATER_STILL) {
      topY = wy + 1.0f;
    } else {
      uint8_t wl = m_level->getWaterLevel(wX, wY, wZ);
      if (wl <= 2) topY = wy + 0.75f;
      else if (wl <= 4) topY = wy + 0.875f;
      else topY = wy + 1.0f;
      uint8_t above = m_level->getBlock(wX, wY + 1, wZ);
      if (above == BLOCK_WATER_STILL || above == BLOCK_WATER_FLOW) topY = wy + 1.0f;
    }
    topY_NW = topY;
    topY_NE = topY;
    topY_SW = topY;
