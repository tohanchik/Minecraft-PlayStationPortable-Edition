...
bool TileRenderer::tesselateBlockInWorld(uint8_t id, int lx, int ly, int lz, int cx, int cz) {
  ...
  if (id == BLOCK_WATER_STILL || id == BLOCK_WATER_FLOW) {
    ...
    // Depth-driven corner heights so flowing water visually slopes while spreading/drying.
    auto sampleHeight = [&](int sx, int sz) -> float {
      ...
      if (d == 0xFF || d > 7) return 0.0f;
      return 1.0f - ((float)d / 8.0f);
    };

    float h00 = sampleHeight(wX, wZ);
    float h01 = sampleHeight(wX, wZ + 1);
    float h11 = sampleHeight(wX + 1, wZ + 1);
    float h10 = sampleHeight(wX + 1, wZ);
    bool drawn = false;
    ...
    if (needFace(lx, ly, lz, cx, cz, id, 0, 1, 0, isFancy)) {
      ...
      uint32_t c = applyLightToFace(0xCCFFFFFF, br);
    ...
    if (needFace(lx, ly, lz, cx, cz, id, 0, -1, 0, isFancy)) {
      ...
      float bl = getVertexBlockLight(wX, wY - 1, wZ, 0, 0, 0, 0, 0, 0);
      float br = (bl > sl + 0.05f) ? bl : sl;
      uint32_t c = applyLightToFace(0xCCB0B0B0, br);
    ...
    auto addSide = [&](int dx, int dz) {
      ...
      float br = (bl > sl + 0.05f) ? bl : sl;
      uint32_t c = applyLightToFace(0xCCDDDDDD, br * 0.85f);
