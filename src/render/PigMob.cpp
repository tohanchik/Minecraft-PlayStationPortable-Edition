...
static void spawnNearPlayer(float playerX, float playerY, float playerZ, float playerYawDeg) {
  ...
  // Spawn right in front of the player for guaranteed visibility on start.
  g_pig.x = playerX + forwardX * 2.25f;
  g_pig.z = playerZ + forwardZ * 2.25f;
  g_pig.y = playerY;
...
void PigMob_Render() {
  ...
  ScePspFVector3 rot = {0.0f, -(g_pig.yawDeg * (3.14159265f / 180.0f)), 0.0f};
  sceGumRotateYXZ(&rot);
  ...
  unsigned int side = g_pig.hasPigTexture ? 0xFF8FA8D6u : 0xFF92B3FFu;
  unsigned int top =  g_pig.hasPigTexture ? 0xFFA8C0EAu : 0xFFABC8FFu;
  unsigned int bot =  0xFF6A7DA5u;
