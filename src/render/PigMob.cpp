...
void PigMob_Update(PigMob *pig, Level *level, float dt, float playerX, float playerY,
                   float playerZ) {
  // Keep pig close to player so it is always visible/spawned near you.
  float dxToPlayer = pig->x - playerX;
  float dzToPlayer = pig->z - playerZ;
  if (dxToPlayer * dxToPlayer + dzToPlayer * dzToPlayer > (10.0f * 10.0f)) {
    pig->x = playerX + 1.5f;
    pig->z = playerZ + 1.5f;
    pig->y = findGroundY(level, pig->x, pig->z);
    pig->vx = pig->vy = pig->vz = 0.0f;
