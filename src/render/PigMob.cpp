...
void PigMob_Update(PigMob *pig, Level *level, float dt, float playerX, float playerY,
                   float playerZ) {
  // Hard-follow mode: keep pig always near the player so it's never lost.
  float desiredX = playerX + 1.8f;
  float desiredZ = playerZ + 1.8f;

  float toX = desiredX - pig->x;
  float toZ = desiredZ - pig->z;
  float dist2ToAnchor = toX * toX + toZ * toZ;

  // If pig got far away, snap immediately near player.
  if (dist2ToAnchor > (12.0f * 12.0f)) {
    pig->x = desiredX;
    pig->z = desiredZ;
    pig->y = findGroundY(level, pig->x, pig->z);
    pig->vx = pig->vy = pig->vz = 0.0f;
    toX = desiredX - pig->x;
    toZ = desiredZ - pig->z;
    dist2ToAnchor = toX * toX + toZ * toZ;
  }

  // Follow the player's anchor point.
  float distToAnchor = sqrtf(dist2ToAnchor);
  if (distToAnchor > 0.05f) {
    pig->targetYawDeg = atan2f(toX, toZ) / Mth::DEGRAD;
    pig->moveSpeed = clampf(distToAnchor * 2.0f, 0.30f, 1.75f);
  } else {
    pig->moveSpeed = 0.0f;
  }
