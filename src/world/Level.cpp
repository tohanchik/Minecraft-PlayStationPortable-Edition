...
void Level::tickLiquid(bool lava) {
  ...
  for (int y = simMaxY; y >= simMinY && !budgetReached; --y) {
    for (int z = simMinZ; z <= simMaxZ; ++z) {
      for (int x = simMinX; x <= simMaxX; ++x) {
        ...
        auto queueLavaCooling = [&](int tx, int ty, int tz) {
          if (!inBounds(tx, ty, tz)) return;
          uint8_t lavaId = getBlock(tx, ty, tz);
          if (!isLavaBlock(lavaId)) return;
          uint8_t lavaDepth = getWaterDepth(tx, ty, tz);
          if (lavaDepth == 0xFF) lavaDepth = (lavaId == BLOCK_LAVA_STILL) ? 0 : 2;
          queueSet(tx, ty, tz, (lavaDepth == 0) ? BLOCK_OBSIDIAN : BLOCK_COBBLESTONE, 0xFF);
        };
        ...
        if (lava) {
          ...
          // Water instantly cools adjacent lava (except above, to preserve
          // lava-falls-into-water -> stone behavior handled in lava ticks).
          queueLavaCooling(x - 1, y, z);
          queueLavaCooling(x + 1, y, z);
          queueLavaCooling(x, y, z - 1);
          queueLavaCooling(x, y, z + 1);
          queueLavaCooling(x, y - 1, z);
        }
        ...
        for (int i = 0; i < 4; ++i) {
          ...
          if (isSameLiquid(nId)) {
            ...
            if (nd == 0xFF) nd = (nId == stillId) ? 0 : dropOff;
  ...
  if (lava) m_lavaDirty = budgetReached || !ops.empty();
  else m_waterDirty = budgetReached || !ops.empty();
}
...
void Level::setBlock(int wx, int wy, int wz, uint8_t id) {
  ...
  if (!touchesWater || !touchesLava) {
    ...
    for (int i = 0; i < 6; ++i) {
      ...
      if (!touchesWater && isWaterBlock(n)) touchesWater = true;
      if (!touchesLava && isLavaBlock(n)) touchesLava = true;
      if (touchesWater && touchesLava) break;
    }
  }
  if (touchesWater || touchesLava) {
    if (touchesWater) m_waterDirty = true;
    if (touchesLava) m_lavaDirty = true;
    m_waterWakeX = wx;
    m_waterWakeY = wy;
    m_waterWakeZ = wz;
    m_waterWakeTicks = 16;
