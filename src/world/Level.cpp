      ...
      for (int x = simMinX; x <= simMaxX; ++x) {
        ...
        if (nextDepth <= 7) {
          ...
          if (spreadDepth <= 7 && !flowedDown) {
            // Keep expensive path search on dynamic flow fronts. For source-level
            // spread, enable it only in the local wake window after nearby edits.
            bool useShortestPathPriority =
                (id == BLOCK_WATER_FLOW) || (spreadDepth == 1 && m_waterWakeTicks > 0);
