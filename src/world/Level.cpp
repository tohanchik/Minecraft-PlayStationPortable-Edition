...
static uint32_t fnv1a32Update(uint32_t hash, const void *data, size_t size) {
...
bool Level::saveToFile(const char *path) const {
  ...
  if (fwrite(&hdr, sizeof(hdr), 1, f) != 1) { fclose(f); remove(tmpPath); return false; }
  checksum = fnv1a32Update(checksum, &hdr.time, sizeof(hdr.time));

  for (int cx = 0; cx < WORLD_CHUNKS_X; ++cx) {
    for (int cz = 0; cz < WORLD_CHUNKS_Z; ++cz) {
      const Chunk *ch = m_chunks[cx][cz];
      if (fwrite(ch->blocks, sizeof(ch->blocks), 1, f) != 1) { fclose(f); remove(tmpPath); return false; }
      if (fwrite(ch->light, sizeof(ch->light), 1, f) != 1) { fclose(f); remove(tmpPath); return false; }
      checksum = fnv1a32Update(checksum, ch->blocks, sizeof(ch->blocks));
      checksum = fnv1a32Update(checksum, ch->light, sizeof(ch->light));
    }
  }
  ...
  if (!m_waterDepth.empty()) {
    ...
    checksum = fnv1a32Update(checksum, m_waterDepth.data(), m_waterDepth.size());
  }
...
bool Level::loadFromFile(const char *path) {
  ...
  if (hdr.version >= 2) {
    ...
    if (fread(&storedChecksum, sizeof(storedChecksum), 1, f) != 1) { fclose(f); return false; }
    if (storedChecksum != checksum) { fclose(f); return false; }
  }
