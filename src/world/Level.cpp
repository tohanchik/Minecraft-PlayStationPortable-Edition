      ...
      if (spreadDepth <= 7 && !flowedDown) {
        ...
        for (int i = 0; i < 4; ++i) {
          ...
          if (useShortestPathPriority) {
            ...
            // Source/still blocks should not carpet-spread on flat tops (e.g. trees).
            // They may only hand off sideways if that neighbor can immediately go down.
            costs[i] = hasDownwardExit(nx, y, nz) ? 0 : 999;
          }
