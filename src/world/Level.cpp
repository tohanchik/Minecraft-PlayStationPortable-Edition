  ...
  for (int y = simMaxY; y >= simMinY && !budgetReached; --y) {
    for (int z = simMinZ; z <= simMaxZ; ++z) {
      for (int x = simMinX; x <= simMaxX; ++x) {
        ...
        // Water/lava reactions are applied only on actual touching neighbors.
        ...
        for (int i = 0; i < 6; ++i) {
          ...
          if (!isLavaBlock(nId)) continue;
          uint8_t ld = getLavaDepth(nx, ny, nz);
          if (ld == 0xFF) ld = (nId == BLOCK_LAVA_STILL) ? 0 : 1;
          queueSet(nx, ny, nz, (nId == BLOCK_LAVA_STILL || ld == 0) ? BLOCK_OBSIDIAN : BLOCK_COBBLESTONE, 0xFF);
        }
