  ...
  for (int y = simMaxY; y >= simMinY && !budgetReached; --y) {
    for (int z = simMinZ; z <= simMaxZ; ++z) {
      for (int x = simMinX; x <= simMaxX; ++x) {
        ...
        uint8_t curDepth = getWaterDepth(x, y, z);
        if (curDepth == 0xFF) curDepth = (id == stillId) ? 0 : 1;
        ...
        if (id == stillId) {
          ...
          if (hasLiquidAbove) nextDepth = 1;
          else if (sourceNeighbors >= 2 && supportBelow) nextDepth = 0;
          else if (neighborMin < 7) nextDepth = (uint8_t)(neighborMin + (lava ? 2 : 1));
          else nextDepth = 8;
        }

        if (nextDepth <= 7) {
          uint8_t spreadDepth = (id == stillId) ? 1 : (uint8_t)(nextDepth + (lava ? 2 : 1));
