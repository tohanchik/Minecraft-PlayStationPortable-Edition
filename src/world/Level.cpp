  ...
  for (int y = CHUNK_SIZE_Y - 1; y >= 0; --y) {
    for (int z = 0; z < maxZ; ++z) {
      for (int x = 0; x < maxX; ++x) {
        ...
        bool hasSourceNeighbor = false;
        bool hasWaterNeighbor = false;
        if (isWaterBlock(getBlock(x, y + 1, z))) hasSourceNeighbor = true;
        for (int i = 0; i < 4; ++i) {
          ...
          if (isWaterBlock(nId)) {
            ...
            if (nId == BLOCK_WATER_STILL || nd == 0) hasSourceNeighbor = true;
          }
        }
        ...
        if (id == BLOCK_WATER_STILL) {
          ...
          if (hasSourceNeighbor) nextDepth = 1;
        ...
        if (nextDepth <= 7) {
          ...
          if (id == BLOCK_WATER_FLOW) {
            if (nextDepth >= 8 || (!hasSourceNeighbor && !hasWaterNeighbor && !flowedDown)) {
