...
void Level::tick() {
  m_time += 1;
  // MCPE-like throttling, but boost cadence when active liquid is near player focus.
  int waterInterval = 5;
  int lavaInterval = 30;
  ...
  if (m_simFocusX >= 0 && m_simFocusY >= 0 && m_simFocusZ >= 0) {
    auto queueNearFocus = [&](const std::deque<int> &q, int checkCount, int radiusXZ, int radiusY) -> bool {
      int checked = 0;
      for (int idx : q) {
        if (checked++ >= checkCount) break;
        int x = idx % maxX;
        int yz = idx / maxX;
        int z = yz % maxZ;
        int y = yz / maxZ;
        int dx = x - m_simFocusX; if (dx < 0) dx = -dx;
        int dy = y - m_simFocusY; if (dy < 0) dy = -dy;
        int dz = z - m_simFocusZ; if (dz < 0) dz = -dz;
        if (dx <= radiusXZ && dz <= radiusXZ && dy <= radiusY) return true;
      }
      return false;
    };
    if (!m_waterQueue.empty() && queueNearFocus(m_waterQueue, 96, 20, 12)) waterInterval = 1;
    if (!m_lavaQueue.empty() && queueNearFocus(m_lavaQueue, 64, 20, 12)) lavaInterval = 6;
  }

  if (m_waterDirty && ++m_waterTickAccum >= waterInterval) {
    m_waterTickAccum = 0;
    tickWater();
    if (m_waterWakeTicks > 0) m_waterWakeTicks--;
  }
  if (m_lavaDirty && ++m_lavaTickAccum >= lavaInterval) {
    m_lavaTickAccum = 0;
    tickLava();
...
void Level::tickWater() {
  ...
  int seeds = 0;
  const int maxSeedsPerTick = 64;
  std::vector<int> consumedSeeds;
  consumedSeeds.reserve(maxSeedsPerTick);
  ...
  if (budgetReached) {
    for (int idx : consumedSeeds) {
      if (idx < 0 || idx >= (int)m_waterQueued.size()) continue;
      if (m_waterQueued[idx]) continue;
      m_waterQueued[idx] = 1;
      m_waterQueue.push_back(idx);
    }
  }
  m_waterDirty = budgetReached || !m_waterQueue.empty() || !ops.empty();
}
...
void Level::tickLava() {
  ...
  if (budgetReached) {
    for (int idx : consumedSeeds) {
      if (idx < 0 || idx >= (int)m_lavaQueued.size()) continue;
      if (m_lavaQueued[idx]) continue;
      m_lavaQueued[idx] = 1;
      m_lavaQueue.push_back(idx);
    }
  }
  m_lavaDirty = budgetReached || !m_lavaQueue.empty() || !ops.empty();
}
