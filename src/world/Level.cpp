...
void Level::tick() {
  ...
  if (m_lavaDirty && ++m_lavaTickAccum >= 18) {
    m_lavaTickAccum = 0;
    tickLava();
...
void Level::tickWater() {
  ...
  auto canFlowInto = [&](int x, int y, int z) {
    ...
    if (isLavaBlock(t)) {
      uint8_t ld = getLavaDepth(x, y, z);
      if (ld == 0xFF) ld = (t == BLOCK_LAVA_STILL) ? 0 : 1;
      queueSet(x, y, z, (t == BLOCK_LAVA_STILL || ld == 0) ? BLOCK_OBSIDIAN : BLOCK_COBBLESTONE, 0xFF);
      return false;
    }
...
void Level::tickLava() {
  ...
  ops.reserve(2048);
  const int maxLavaCellsPerTick = 512;
  ...
  auto canFlowInto = [&](int x, int y, int z) {
    ...
    if (isWaterBlock(t)) {
      queueSet(x, y, z, BLOCK_STONE, 0xFF);
      return false;
    }
    return !g_blockProps[t].isSolid();
  };
  ...
  for (int y = simMaxY; y >= simMinY && !budgetReached; --y) {
    for (int z = simMinZ; z <= simMaxZ; ++z) {
      for (int x = simMinX; x <= simMaxX; ++x) {
        ...
        uint8_t belowId = getBlock(x, y - 1, z);
        bool supportBelow = (y == 0) || g_blockProps[belowId].isSolid() || isLavaBlock(belowId);
        uint8_t nextDepth = curDepth;
        ...
        if (nextDepth <= 7) {
          uint8_t spreadDepth = (id == BLOCK_LAVA_STILL) ? 1 : (uint8_t)(nextDepth + 2);
          if (spreadDepth <= 7 && !flowedDown) {
            for (int i = 0; i < 4; ++i) {
              int nx = x + flowDx[i], nz = z + flowDz[i];
              if (!inBounds(nx, y, nz)) continue;
