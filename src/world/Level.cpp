    ...
    if (nextDepth <= 7) {
      ...
      if (id == BLOCK_WATER_FLOW) {
        if (nextDepth >= 8 || (!hasWaterAbove && !hasWaterNeighbor && !flowedDown)) {
          ...
          // MCPE dynamic liquid keeps ticking while unsupported; otherwise it can
          // go static. Re-tick changing cells, and also unsupported non-source
          // cells even if depth is temporarily unchanged.
          keepDynamicTick = (nextDepth > 0) &&
                            (nextDepth != curDepth || (!hasWaterAbove && sourceNeighbors == 0));
        }
      } else {
