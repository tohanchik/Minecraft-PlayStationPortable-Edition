    ...
    if (nextDepth <= 7) {
      ...
      if (spreadDepth <= 7 && !flowedDown) {
        ...
        for (int i = 0; i < 4; ++i) {
          ...
          // No immediate drop on this side. Allow a limited lateral fallback only
          // onto stable ground (opaque support below) so water can still spread on
          // terrain, but does not carpet-flood over tree leaves.
          uint8_t nId = getBlock(nx, y, nz);
          if (!isWaterBlock(nId)) {
            int ny = y - 1;
            if (ny >= 0) {
              uint8_t underId = getBlock(nx, ny, nz);
              if (fallbackDir < 0 && g_blockProps[underId].isOpaque()) {
                fallbackDir = i;
              }
            }
          }
