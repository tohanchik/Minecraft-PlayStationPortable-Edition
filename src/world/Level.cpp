...
void Level::processWaterCell(int x, int y, int z) {
  ...
  if (depth > 0) {
    ...
    bool supportBelow = (y == 0) || g_blockProps[belowId].isSolid() ||
                        (isWaterBlock(belowId) && getWaterDepth(x, y - 1, z) == 0);
    if (sourceCount >= 2 && supportBelow) newDepth = 0;
  ...
  // MCPE-like: unchanged dynamic liquid goes static/sleeping.
  if (id == BLOCK_WATER_FLOW && newDepth == (int)depth && newDepth > 0) {
    nextId = BLOCK_WATER_STILL;
  }
  ...
  if (!spreadDown) {
    ...
    if (spreadDepth <= 7) {
      ...
      auto hasDownwardExit = [&](int wx, int wy, int wz) {
        return wy > 0 && canFlowInto(wx, wy - 1, wz) && !isWaterBlock(getBlock(wx, wy - 1, wz));
      };
      std::function<int(int, int, int, int, int)> flowCost =
          [&](int wx, int wy, int wz, int dist, int fromDir) -> int {
        const int searchLimit = 7;
        if (hasDownwardExit(wx, wy, wz)) return dist;
        if (dist >= searchLimit) return 999;
        int best = 999;
        for (int d = 0; d < 4; ++d) {
          if (fromDir >= 0 && d == opposite[fromDir]) continue;
          int nx = wx + dx[d], nz = wz + dz[d];
          if (!canFlowInto(nx, wy, nz)) continue;
          int c = flowCost(nx, wy, nz, dist + 1, d);
          if (c < best) best = c;
        }
        return best;
      };
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        if (!isWaterBlock(nId) || nDepth == 0xFF || nDepth > spreadDepth) {
          ...
          int ny = y - 1;
          if (ny >= 0 && canFlowInto(nx, ny, nz) && !isWaterBlock(getBlock(nx, ny, nz))) {
            m_inWaterSimUpdate = true;
            setBlock(nx, ny, nz, BLOCK_WATER_FLOW);
            m_inWaterSimUpdate = false;
            setWaterDepth(nx, ny, nz, 1);
            scheduleWaterTick(nx, ny, nz, 5);
          }
        }
      }
    }
