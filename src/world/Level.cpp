...
void Level::processWaterCell(int x, int y, int z) {
  ...
  if (depth > 0) {
    ...
    if (newDepth != depth) {
      if (newDepth < 0) {
        // Smooth dry-out: remove one flow layer per liquid tick.
        if (depth < 7) {
          int nextDepth = depth + 1;
          setWaterCell(x, y, z, BLOCK_WATER_FLOW, (uint8_t)nextDepth);
      ...
      // Avoid abrupt jumps to thin layers in one tick.
      if (newDepth > depth + 1) newDepth = depth + 1;
      depth = newDepth;
