...
void Level::tick() {
  m_time += 1;
  // Fixed cadence for stable liquid timing (avoids queue-driven cadence jitter).
...
void Level::tickWater() {
  ...
  int simMinZ = 0;
  int simMaxZ = maxZ - 1;
  // Dequeue bookkeeping, but compute deterministic simulation bounds from focus/wake.
  while (!m_waterQueue.empty()) {
    int idx = m_waterQueue.front();
    m_waterQueue.pop_front();
    if (idx >= 0 && idx < (int)m_waterQueued.size()) m_waterQueued[idx] = 0;
  }
  if (m_simFocusX >= 0 && m_simFocusY >= 0 && m_simFocusZ >= 0) {
    int r = (m_simFocusRadius > 0) ? m_simFocusRadius : 16;
    int ry = m_simFocusYRadius > 0 ? m_simFocusYRadius : 16;
    simMinX = std::max(0, m_simFocusX - r);
    simMaxX = std::min(maxX - 1, m_simFocusX + r);
    simMinZ = std::max(0, m_simFocusZ - r);
    simMaxZ = std::min(maxZ - 1, m_simFocusZ + r);
    simMinY = std::max(0, m_simFocusY - ry);
    simMaxY = std::min(CHUNK_SIZE_Y - 1, m_simFocusY + ry);
  }
  if (m_waterWakeTicks > 0 && m_waterWakeX >= 0 && m_waterWakeY >= 0 && m_waterWakeZ >= 0) {
    const int wr = m_waterWakeRadius;
    int wx0 = std::max(0, m_waterWakeX - wr);
    int wx1 = std::min(maxX - 1, m_waterWakeX + wr);
    int wz0 = std::max(0, m_waterWakeZ - wr);
    int wz1 = std::min(maxZ - 1, m_waterWakeZ + wr);
    int wy0 = std::max(0, m_waterWakeY - wr);
    int wy1 = std::min(CHUNK_SIZE_Y - 1, m_waterWakeY + wr);
    simMinX = std::min(simMinX, wx0);
    simMaxX = std::max(simMaxX, wx1);
    simMinZ = std::min(simMinZ, wz0);
    simMaxZ = std::max(simMaxZ, wz1);
    simMinY = std::min(simMinY, wy0);
    simMaxY = std::max(simMaxY, wy1);
  }
  ...
  std::unordered_map<uint64_t, int> flowCostCache;
  flowCostCache.reserve(4096);
  auto packFlowKey = [&](int x, int y, int z, int dist, int fromDir) -> uint64_t {
    // MCPE-style bounded search (pass < 4): cache recursive slope checks
    // to avoid recomputing the same branch many times in one tick.
    uint64_t ux = (uint64_t)(x & 0x3FF);
    uint64_t uy = (uint64_t)(y & 0xFF);
    uint64_t uz = (uint64_t)(z & 0x3FF);
    uint64_t ud = (uint64_t)(dist & 0x0F);
    uint64_t uf = (uint64_t)((fromDir + 1) & 0x07);
    return ux | (uz << 10) | (uy << 20) | (ud << 28) | (uf << 32);
  };

  std::function<int(int, int, int, int, int)> flowCost =
      [&](int x, int y, int z, int dist, int fromDir) -> int {
    uint64_t key = packFlowKey(x, y, z, dist, fromDir);
    auto cached = flowCostCache.find(key);
    if (cached != flowCostCache.end()) return cached->second;
    if (hasDownwardExit(x, y, z)) return dist;
    if (dist >= maxFlowSearch) {
      flowCostCache[key] = 999;
      return 999;
    }
    ...
    flowCostCache[key] = best;
    return best;
  };

  bool reverseX = (m_waterScanPhase & 1) != 0;
  bool reverseZ = (m_waterScanPhase & 2) != 0;
  bool reverseY = (m_waterScanPhase & 4) != 0;
  int yStart = reverseY ? simMinY : simMaxY;
  int yEnd = reverseY ? simMaxY : simMinY;
  int yStep = reverseY ? 1 : -1;
  int zStart = reverseZ ? simMaxZ : simMinZ;
  int zEnd = reverseZ ? simMinZ : simMaxZ;
  int zStep = reverseZ ? -1 : 1;
  int xStart = reverseX ? simMaxX : simMinX;
  int xEnd = reverseX ? simMinX : simMaxX;
  int xStep = reverseX ? -1 : 1;
  for (int y = yStart; !budgetReached && (reverseY ? (y <= yEnd) : (y >= yEnd)); y += yStep) {
      ...
      for (int x = xStart; reverseX ? (x >= xEnd) : (x <= xEnd); x += xStep) {
        ...
        if (nextDepth <= 7) {
          ...
          if (spreadDepth <= 7 && !flowedDown) {
            ...
            bool immediateDown[4] = {false, false, false, false};
            bool hasImmediateDown = false;
            for (int i = 0; i < 4; ++i) {
              int nx = x + flowDx[i], nz = z + flowDz[i];
              if (!inBounds(nx, y, nz)) continue;
              if (!canFlowInto(nx, y, nz)) continue;
              canSpread[i] = true;
              immediateDown[i] = hasDownwardExit(nx, y, nz);
              if (immediateDown[i]) {
                hasImmediateDown = true;
                costs[i] = 0;
                bestCost = 0;
              }
            }
            // MCPE-equivalent optimization: if any adjacent branch already has
            // an immediate downward exit, slope-path recursion is unnecessary
            // because cost 0 is globally optimal.
            if (!hasImmediateDown) {
              for (int i = 0; i < 4; ++i) {
                if (!canSpread[i]) continue;
                int nx = x + flowDx[i], nz = z + flowDz[i];
                costs[i] = flowCost(nx, y, nz, 1, i);
  ...
  m_waterDirty = budgetReached || !ops.empty() || (m_waterWakeTicks > 0);
  m_waterScanPhase = (m_waterScanPhase + 1) & 7;
}
...
void Level::tickLava() {
  ...
  const int maxLavaCellsPerTick = 192;
  ...
  int simMinZ = 0;
  int simMaxZ = maxZ - 1;
  while (!m_lavaQueue.empty()) {
    int idx = m_lavaQueue.front();
    m_lavaQueue.pop_front();
    if (idx >= 0 && idx < (int)m_lavaQueued.size()) m_lavaQueued[idx] = 0;
  }
  if (m_simFocusX >= 0 && m_simFocusY >= 0 && m_simFocusZ >= 0) {
    int r = (m_simFocusRadius > 0) ? m_simFocusRadius : 16;
    int ry = m_simFocusYRadius > 0 ? m_simFocusYRadius : 16;
  ...
  if (m_lavaWakeTicks > 0 && m_lavaWakeX >= 0 && m_lavaWakeY >= 0 && m_lavaWakeZ >= 0) {
    const int wr = m_lavaWakeRadius;
    int wx0 = std::max(0, m_lavaWakeX - wr);
    int wx1 = std::min(maxX - 1, m_lavaWakeX + wr);
    int wz0 = std::max(0, m_lavaWakeZ - wr);
    int wz1 = std::min(maxZ - 1, m_lavaWakeZ + wr);
    int wy0 = std::max(0, m_lavaWakeY - wr);
    int wy1 = std::min(CHUNK_SIZE_Y - 1, m_lavaWakeY + wr);
    simMinX = std::min(simMinX, wx0);
    simMaxX = std::max(simMaxX, wx1);
    simMinZ = std::min(simMinZ, wz0);
    simMaxZ = std::max(simMaxZ, wz1);
  ...
  bool reverseX = (m_lavaScanPhase & 1) != 0;
  bool reverseZ = (m_lavaScanPhase & 2) != 0;
  bool reverseY = (m_lavaScanPhase & 4) != 0;
  int yStart = reverseY ? simMinY : simMaxY;
  int yEnd = reverseY ? simMaxY : simMinY;
  int yStep = reverseY ? 1 : -1;
  int zStart = reverseZ ? simMaxZ : simMinZ;
  ...
  m_lavaDirty = budgetReached || !ops.empty() || (m_lavaWakeTicks > 0);
  m_lavaScanPhase = (m_lavaScanPhase + 1) & 7;
}
...
void Level::enqueueWaterCell(int wx, int wy, int wz, bool highPriority) {
  ...
  m_waterQueued[idx] = 1;
  if (highPriority) m_waterQueue.push_front(idx);
  else m_waterQueue.push_back(idx);
  m_waterDirty = true;
}

void Level::enqueueLavaCell(int wx, int wy, int wz, bool highPriority) {
  int maxX = WORLD_CHUNKS_X * CHUNK_SIZE_X;
  int maxZ = WORLD_CHUNKS_Z * CHUNK_SIZE_Z;
  if (wx < 0 || wx >= maxX || wz < 0 || wz >= maxZ || wy < 0 || wy >= CHUNK_SIZE_Y) return;
  ...
  m_lavaQueued[idx] = 1;
  if (highPriority) m_lavaQueue.push_front(idx);
  else m_lavaQueue.push_back(idx);
  m_lavaDirty = true;
}
...
void Level::setBlock(int wx, int wy, int wz, uint8_t id) {
  ...
  if (touchesWater) {
    ...
    for (int dx = -1; dx <= 1; ++dx)
      for (int dy = -1; dy <= 1; ++dy)
        for (int dz = -1; dz <= 1; ++dz)
          if ((dx == 0) + (dy == 0) + (dz == 0) >= 2) enqueueWaterCell(wx + dx, wy + dy, wz + dz, true);
  }
  if (touchesLava) {
    m_lavaDirty = true;
    m_lavaWakeX = wx;
    ...
    for (int dx = -1; dx <= 1; ++dx)
      for (int dy = -1; dy <= 1; ++dy)
        for (int dz = -1; dz <= 1; ++dz)
          if ((dx == 0) + (dy == 0) + (dz == 0) >= 2) enqueueLavaCell(wx + dx, wy + dy, wz + dz, true);
  }
...
bool Level::loadFromFile(const char *path) {
  ...
  for (int y = 0; y < CHUNK_SIZE_Y; ++y) {
    for (int z = 0; z < WORLD_CHUNKS_Z * CHUNK_SIZE_Z; ++z) {
      for (int x = 0; x < WORLD_CHUNKS_X * CHUNK_SIZE_X; ++x) {
        uint8_t id = getBlock(x, y, z);
        if (id == BLOCK_WATER_FLOW) enqueueWaterCell(x, y, z);
        if (id == BLOCK_LAVA_FLOW) enqueueLavaCell(x, y, z);
      }
    }
  }
  m_waterDirty = !m_waterQueue.empty();
  m_lavaDirty = !m_lavaQueue.empty();
  return true;
}
