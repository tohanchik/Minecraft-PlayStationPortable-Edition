...
float WorldGen::getTemperature(int wx, int wz, int64_t seed) {
  float t = 0.5f;
  float d = 0.5f;
  int biome = BIOME_PLAINS;
  getClimateAt(wx, wz, seed, t, d, biome);
...
void WorldGen::getTemperatureBlock(float out[16][16], int x, int z,
                                   int64_t seed) {
  for (int xx = 0; xx < 16; ++xx) {
    for (int zz = 0; zz < 16; ++zz) {
      out[xx][zz] = getTemperature(x + xx, z + zz, seed);
    }
  }
}
      ...
      for (int y = CHUNK_SIZE_Y - 1; y >= 0; --y) {
        ...
        if (run == -1) {
          if (runDepth <= 0) {
            ...
            if (useGravel) {
              // Keep a solid shoreline cap for gravel coasts; leaving top as AIR
              // below sea level turns the first stone layer into water and creates
              // frequent "sunken banks" artifacts.
              top = BLOCK_GRAVEL;
              filler = BLOCK_GRAVEL;
            }
            if (useSand) {
