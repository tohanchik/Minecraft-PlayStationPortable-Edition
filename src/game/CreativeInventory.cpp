...
CreativeInventory::CreativeInventory()
  : m_open(false), m_hotbarSel(0), m_cursorX(0), m_cursorY(0), m_creativePage(0),
    m_usingSlider(false), m_cursorHasItem(false), m_cursorItem(BLOCK_AIR) {
...
void CreativeInventory::open() { m_open = true; m_cursorX = 0; m_cursorY = 0; }
...
void CreativeInventory::pressCross() {
  ...
  if (m_cursorX < 4 && idx >= 0 && idx < inventoryItemCount() && !m_cursorHasItem) {
    m_cursorItem = kInventoryItems[idx];
    m_cursorHasItem = true;
  }
}
...
int CreativeInventory::maxPage() const { return inventoryItemCount() / 25; }
