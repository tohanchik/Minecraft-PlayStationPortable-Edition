...
static inline uint32_t hudColorForBlock(uint8_t id) {
  uint32_t c = (uint32_t)(id * 1103515245u + 12345u);
  uint8_t r = 60 + (c & 0x7F);
  uint8_t g = 60 + ((c >> 8) & 0x7F);
  uint8_t b = 60 + ((c >> 16) & 0x7F);
  return 0xC0000000u | ((uint32_t)b << 16) | ((uint32_t)g << 8) | r;
}
...
static void drawHotbarHUD() {
  ...
  for (int i = 0; i < 9; ++i) {
    ...
    hudDrawRect(sx + 3, y + 3, slot - 6, slot - 6, hudColorForBlock(id));
  }
  ...
  if (g_inventoryOpen) {
    ...
    for (int r = 0; r < rows; ++r) {
      for (int c = 0; c < cols; ++c) {
        ...
        hudDrawRect(sx + 3, sy + 3, cell - 9, cell - 9, hudColorForBlock(id));
      }
    }
    ...
    if (g_pendingInventoryItem >= 0) {
      ...
      hudDrawRect(px, py, 24, 24, hudColorForBlock((uint8_t)g_inventoryItems[g_pendingInventoryItem]));
    }
