...
static float g_invTitleOffsetX = 0.0f;
static float g_invTitleOffsetY = 0.0f;
static float g_invTuneToggleComboTimer = 0.0f;
static bool g_invTuneToggleComboArmed = false; // UP then TRIANGLE
static float g_invTuneTargetComboTimer = 0.0f;
static bool g_invTuneTargetComboArmed = false; // DOWN then TRIANGLE
...
static void drawHotbarHUD() {
  ...
  if (g_creativeInv.isOpen()) {
    ...
    if (g_invTuneMode) {
      ...
      const char *tuneLabel = "TUNE GRID (DOWN->TRIANGLE)";
      if (g_invTuneTarget == 1) tuneLabel = "TUNE HOTBAR (DOWN->TRIANGLE)";
      else if (g_invTuneTarget == 2) tuneLabel = "TUNE DELETE (DOWN->TRIANGLE)";
      else if (g_invTuneTarget == 3) tuneLabel = "TUNE TITLE (DOWN->TRIANGLE)";
...
static void game_update(float dt) {
  ...
  if (g_creativeInv.isOpen() && PSPInput_JustPressed(PSP_CTRL_CIRCLE)) {
    if (g_creativeInv.cursorHasItem()) {
      ...
      g_invTuneMode = false;
      g_invTuneTarget = 0;
      g_invTuneToggleComboArmed = false;
      g_invTuneToggleComboTimer = 0.0f;
      g_invTuneTargetComboArmed = false;
      g_invTuneTargetComboTimer = 0.0f;
    }
  }
  ...
  if (g_creativeInv.isOpen()) {
    if (g_invTuneToggleComboTimer > 0.0f) {
      g_invTuneToggleComboTimer -= dt;
      if (g_invTuneToggleComboTimer <= 0.0f) g_invTuneToggleComboArmed = false;
    }
    if (g_invTuneTargetComboTimer > 0.0f) {
      g_invTuneTargetComboTimer -= dt;
      if (g_invTuneTargetComboTimer <= 0.0f) g_invTuneTargetComboArmed = false;
    }

    // Sequential combos to avoid accidental single-button trigger:
    // UP -> TRIANGLE toggles tuning mode.
    // DOWN -> TRIANGLE cycles tuning target.
    if (PSPInput_JustPressed(PSP_CTRL_UP)) {
      g_invTuneToggleComboArmed = true;
      g_invTuneToggleComboTimer = 0.55f;
    }
    if (PSPInput_JustPressed(PSP_CTRL_DOWN) && g_invTuneMode) {
      g_invTuneTargetComboArmed = true;
      g_invTuneTargetComboTimer = 0.55f;
    }
    if (PSPInput_JustPressed(PSP_CTRL_TRIANGLE)) {
      if (g_invTuneToggleComboArmed) {
        g_invTuneToggleComboArmed = false;
        g_invTuneToggleComboTimer = 0.0f;
        if (!g_invTuneMode) {
          g_invTuneMode = true;
      ...
      } else if (g_invTuneMode && g_invTuneTargetComboArmed) {
        g_invTuneTargetComboArmed = false;
        g_invTuneTargetComboTimer = 0.0f;
        g_invTuneTarget = (g_invTuneTarget + 1) % 4;
      }
    }
    if (g_invTuneMode) {
