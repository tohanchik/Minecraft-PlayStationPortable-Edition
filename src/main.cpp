...
static float g_invOffsetX = 0.0f;
static float g_invOffsetY = 0.0f;
...
static void drawHotbarHUD() {
  ...
  if (g_creativeInv.isOpen()) {
    ...
    const float hotbarY = 209.0f + g_invOffsetY;
    ...
    if (g_invTuneMode) {
      ...
      hudDrawText5x7(90.0f, 245.0f, "TUNE: DPAD=WARP L/R=SPACING STICK=MOVE TRI=SAVE", 0xFFFFFFFF, 1.0f);
    }
  }
}
...
static void loadInventoryLayoutConfig() {
  ...
  int n = fscanf(f, "%f %f %f %f %f", &step, &sx, &cy, &ox, &oy);
  if (n >= 3) {
