...
static void drawHotbarHUD() {
  ...
  if (g_creativeInv.isOpen()) {
    ...
    const float panelH = 272.0f;
    const float cellStepX = 32.0f;
    const float cellStepY = 31.0f;
    const float cellSize = 30.0f;
    const float cellX0 = 104.0f;
    const float cellY0 = 91.0f;
    const float hotbarY = 224.0f;
    const float sliderX = 430.0f;
    ...
    const float tabX0 = 95.0f;
    const float tabY0 = 47.0f;
    const float tabW = 43.0f;
    const float tabStep = 47.0f;
    hudDrawRect(tabX0 + g_creativeInv.category() * tabStep, tabY0, tabW, 44.0f, 0x50FFFF00);
  }
}
