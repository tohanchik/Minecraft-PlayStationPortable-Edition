...
static void drawHotbarHUD() {
  ...
  if (g_creativeInv.isOpen()) {
    const int invCount = CreativeInventory::inventoryItemCount();
    ...
    for (int r = 0; r < creativeRows; ++r) {
      for (int c = 0; c < creativeCols; ++c) {
        ...
        uint8_t id = CreativeInventory::inventoryItemAt(idx);
        int tx, ty;
        hudGetIconTile(id, tx, ty);
    ...
    if (g_creativeInv.cursorY() < 5 && g_creativeInv.cursorX() < 4) {
      ...
      if (hover >= 0 && hover < invCount) g_inventoryHoverName = getBlockDisplayName(CreativeInventory::inventoryItemAt(hover));
