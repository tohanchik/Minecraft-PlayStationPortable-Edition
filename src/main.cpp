...
static void drawHotbarHUD() {
  ...
  for (int i = 0; i < 9; ++i) {
    ...
    hudDrawTile(g_atlas, g_blockUV[id].top_x, g_blockUV[id].top_y, sx + 3, y + 3, slot - 6);
  }
  ...
  if (g_inventoryOpen) {
    ...
    for (int r = 0; r < rows; ++r) {
      for (int c = 0; c < cols; ++c) {
        ...
        hudDrawTile(g_atlas, g_blockUV[id].top_x, g_blockUV[id].top_y, sx + 3, sy + 3, cell - 9);
      }
    }
    ...
    if (g_pendingInventoryItem >= 0) {
      ...
      hudDrawTile(g_atlas, g_blockUV[id].top_x, g_blockUV[id].top_y, px, py, 24);
    }
