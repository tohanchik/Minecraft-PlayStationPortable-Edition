...
static const float kAutoSaveIntervalSec = 90.0f;
...
static void backupPathFor(const char *path, char *out, int outSize) {
  if (!out || outSize <= 0) return;
  if (!path) {
    out[0] = '\0';
    return;
  }
  snprintf(out, outSize, "%s.bak", path);
}
...
static bool savePlayerAndInventoryForSlot(int slot) {
  ...
  char bakPath[160];
  backupPathFor(path, bakPath, sizeof(bakPath));
...
static bool loadPlayerAndInventoryForSlot(int slot) {
  char path[128];
  worldMetaPathForSlot(slot, path, sizeof(path));
  if (loadPlayerAndInventoryFromPath(path)) return true;
  char bakPath[160];
  backupPathFor(path, bakPath, sizeof(bakPath));
  return loadPlayerAndInventoryFromPath(bakPath);
}
...
static bool loadWorldSlot(int slot) {
  ...
  if (!loadedMain) {
    char bakPath[160];
    backupPathFor(path, bakPath, sizeof(bakPath));
    if (!g_level->loadFromFile(bakPath)) return false;
    setStatusMessage("Recovered world from backup", 3.0f, 0xFF80FFFF);
  }
...
static void game_update(float dt) {
  ...
  if (g_level) {
    ...
    if (!g_inventoryOpen && !g_pauseOpen) {
      g_autoSaveTimer += dt;
