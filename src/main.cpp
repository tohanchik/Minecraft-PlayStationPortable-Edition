...
static uint32_t blockPreviewColor(uint8_t blockId, uint8_t alpha) {
  if (blockId == BLOCK_AIR) return (alpha << 24);
  BlockUV uv = g_blockUV[blockId];
  uint8_t r = (uint8_t)(40 + uv.top_x * 12);
  uint8_t g = (uint8_t)(50 + uv.top_y * 12);
  uint8_t b = (uint8_t)(90 + ((uv.side_x + uv.side_y) % 8) * 14);
  return ((uint32_t)alpha << 24) | ((uint32_t)b << 16) | ((uint32_t)g << 8) | r;
}
...
static void drawHudBlockIcon(int x, int y, int size, uint8_t blockId, uint8_t alpha = 0xFF) {
  ...
  float u0 = TextureAtlas::tileU(uv.top_x);
  float v0 = TextureAtlas::tileV(uv.top_y);
  float u1 = u0 + TextureAtlas::tileSz();
  float v1 = v0 + TextureAtlas::tileSz();

  HudTexVertex *v = (HudTexVertex *)sceGuGetMemory(2 * sizeof(HudTexVertex));
  v[0].u = u0; v[0].v = v0; v[0].color = ((uint32_t)alpha << 24) | 0x00FFFFFF; v[0].x = (float)x; v[0].y = (float)y; v[0].z = 0.0f;
  v[1].u = u1; v[1].v = v1; v[1].color = ((uint32_t)alpha << 24) | 0x00FFFFFF; v[1].x = (float)(x + size); v[1].y = (float)(y + size); v[1].z = 0.0f;
...
static void game_render_hud() {
  ...
  for (int i = 0; i < visibleSlots; ++i) {
    ...
    uint32_t bg = hasItem ? blockPreviewColor(g_hotbar[i], 0xA0) : 0x50101010;
    if (isSel && !g_inventoryOpen) bg = hasItem ? 0xC0FFD060 : 0x90FFE080;
    if (isSel && g_inventoryDragging) bg = 0xD0FFA040;
  ...
  if (g_inventoryOpen) {
    ...
    for (int i = 0; i < invSlots; ++i) {
      ...
      uint32_t fill = blockPreviewColor(PLACEABLE[i], isCursor ? 0xD0 : 0x90);
    ...
    for (int i = 0; i < invSlots; ++i) {
      ...
      int y = invY + 6 + cy * (invSlotH + invGap) + 1;
      drawHudBlockIcon(x, y, invSlotH - 2, PLACEABLE[i], 0xFF);
    }
    sceGuDisable(GU_TEXTURE_2D);
    ...
    if (g_inventoryDragging && g_dragBlock != BLOCK_AIR) {
      ...
      int px = invX + invW - 26;
      int py = invY + 6;
      drawHudRect(px, py, px + 18, py + 18, blockPreviewColor(g_dragBlock, 0xE0));
      sceGuEnable(GU_TEXTURE_2D);
      g_atlas->bind();
      sceGuTexFunc(GU_TFX_REPLACE, GU_TCC_RGBA);
...
static void game_update(float dt) {
  ...
  if (g_inventoryOpen) {
    ...
    if (PSPInput_JustPressed(PSP_CTRL_CROSS)) {
      if (!g_inventoryDragging) {
        g_dragBlock = PLACEABLE[g_inventoryCursor];
