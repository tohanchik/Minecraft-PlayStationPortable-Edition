...
static void savePlayerAndInventoryForSlot(int slot) {
  char path[128];
  ...
  fwrite(&g_player.x, sizeof(g_player.x), 1, f);
  fwrite(&g_player.y, sizeof(g_player.y), 1, f);
  fwrite(&g_player.z, sizeof(g_player.z), 1, f);
  fwrite(&g_player.yaw, sizeof(g_player.yaw), 1, f);
  fwrite(&g_player.pitch, sizeof(g_player.pitch), 1, f);
  fwrite(&g_player.isFlying, sizeof(g_player.isFlying), 1, f);
  fwrite(&g_hotbarSel, sizeof(g_hotbarSel), 1, f);
  fwrite(g_hotbar, sizeof(g_hotbar[0]), 9, f);

  fclose(f);
}

static void loadPlayerAndInventoryForSlot(int slot) {
  char path[128];
  worldMetaPathForSlot(slot, path, sizeof(path));
  FILE *f = fopen(path, "rb");
  if (!f) return;
  ...
  fread(&g_player.x, sizeof(g_player.x), 1, f);
  fread(&g_player.y, sizeof(g_player.y), 1, f);
  fread(&g_player.z, sizeof(g_player.z), 1, f);
  fread(&g_player.yaw, sizeof(g_player.yaw), 1, f);
  fread(&g_player.pitch, sizeof(g_player.pitch), 1, f);
  fread(&g_player.isFlying, sizeof(g_player.isFlying), 1, f);
  fread(&g_hotbarSel, sizeof(g_hotbarSel), 1, f);
  fread(g_hotbar, sizeof(g_hotbar[0]), 9, f);
...
static bool loadWorldSlot(int slot) {
  ...
  loadPlayerAndInventoryForSlot(slot);
  g_worldSlot = slot;
  return true;
}
...
static void createWorldSlot(int slot) {
  ...
  savePlayerAndInventoryForSlot(slot);
  g_worldSlot = slot;
  refreshWorldSlots();
}
  ...
  if (PSPInput_JustPressed(PSP_CTRL_START) && !g_pauseOpen) {
    ...
    if (PSPInput_JustPressed(PSP_CTRL_CROSS)) {
      if (g_pauseSel == 0) {
        ...
        if (g_level) {
          g_level->saveToFile(path);
          savePlayerAndInventoryForSlot(g_worldSlot);
        }
        g_pauseOpen = false;
      } else {
        char path[128];
