...
static const char *kWorldPath = "ms0:/PSP/GAME/MinecraftPSP/worlds/world0.mcpw";
...
static bool game_init() {
  ...
  if (!g_level->loadFromFile(kWorldPath)) {
    Random rng(12345LL);
...
static void game_update(float dt) {
  ...
  if (g_level && PSPInput_JustPressed(PSP_CTRL_START)) {
    g_level->saveToFile(kWorldPath);
  }
