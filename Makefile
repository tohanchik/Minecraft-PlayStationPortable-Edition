...
package: EBOOT.PBP
	...
	@if command -v python3 >/dev/null 2>&1; then \
		...
		bsdtar -a -cf "$(PACKAGE_ZIP)" EBOOT.PBP res "$(BUILD_INFO_FILE)"; \
	elif command -v busybox >/dev/null 2>&1 && busybox --list 2>/dev/null | grep -qx zip; then \
		busybox zip -r "$(PACKAGE_ZIP)" EBOOT.PBP res "$(BUILD_INFO_FILE)"; \
	elif command -v jar >/dev/null 2>&1; then \
		jar -cf "$(PACKAGE_ZIP)" EBOOT.PBP res "$(BUILD_INFO_FILE)"; \
	else \
		echo "Error: no zip-capable tool found (python3/python/zip/bsdtar/busybox-zip/jar)." >&2; \
		exit 127; \
	fi
